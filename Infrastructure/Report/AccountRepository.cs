...
namespace Infrastructure.Report
{
        ...
        SignInManager<ApplicationUser> signInManager, AppDbContext context) : IAccount
    {
        ...
        public async Task<IEnumerable<GetUserWithRolesResponseDTO>> GetUsersWithRolesAsync()
        {
            var allusers = await userManager.Users.ToListAsync();
            if (allusers is null) return null;

            var List = new List<GetUserWithRolesResponseDTO>();
            foreach (var user in allusers) 
            {
                ...
                List.Add(new GetUserWithRolesResponseDTO()
                {
                    Name = user.Name,
                    Email = user.Email,
        ...
        public async Task<LoginResponse> LoginAccountAsync(LoginDTO model)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(jwtToken) || string.IsNullOrEmpty(refreshtoken))
                    return new LoginResponse(false, "Error occured while logging in account, please contact administraction");
                else
                {
