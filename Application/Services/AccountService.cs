    ...
    public class AccountService(HttpClientService httpClientService) : IAccountService
    {
        ...
        public async Task<IEnumerable<GetUserWithRolesResponseDTO>> GetUsersWithRolesAsync()
        {
            try
            {
                ...
                var result = await response.Content.ReadFromJsonAsync<IEnumerable<GetUserWithRolesResponseDTO>>();
                return result!;
            }
            catch (Exception ex)
            {
        ...
        public async Task<GeneralResponse> CreateRoleAsync(CreateRoleDTO model)
        {
            ...
            {
                var privateClient = await httpClientService.GetPrivateClient();
                var response = await privateClient.PostAsJsonAsync(Constant.CreateRoleRoute, model);
                string error = CheckResponseStatus(response);
                if (!string.IsNullOrEmpty(error))
                    return new GeneralResponse(false, error);
